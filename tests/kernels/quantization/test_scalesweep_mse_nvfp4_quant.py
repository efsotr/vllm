...
def recover_swizzled_scales(scale: torch.Tensor, m: int, n: int) -> torch.Tensor:
    return (
        recover_swizzled_scale_raw(scale, m, n)
...
@pytest.mark.parametrize("device", CUDA_DEVICES)
@torch.inference_mode()
def test_scalesweep_mse128_nvfp4_quant(
    dtype: torch.dtype,
    is_sf_swizzled_layout: bool,
    device: str,
) -> None:
    ...
    x = torch.randn((4, 64), device=device, dtype=dtype, generator=generator)
    global_scale_inv = compute_global_scale_inv(x)
    out_ref, scale_ref = ref_scalesweep_mse_nvfp4_quant_all_positive_fp8(
        x,
        global_scale_inv,
    )
    ...
    direct_out, direct_scale = torch.ops.vllm.scalesweep_mse128_nvfp4_quant(
        x,
        global_scale_inv,
        is_sf_swizzled_layout,
        None,
